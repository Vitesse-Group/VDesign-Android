    ...
    private final Runnable runnable = new Runnable() {
        ...
        public void run() {
            if (PreferenceUtil.getInstance(getApplicationContext()).getBooleanParam(PreferenceUtil.IS_FIRST_TIME) &&
                    !PreferenceUtil.getInstance(getApplicationContext()).getBooleanParam(PreferenceUtil.IS_LOGIN_REMEMBER_ME)) {
                final Intent intent = new Intent(SplashActivity.this, LogInActivity.class);
                startActivity(intent);
                overridePendingTransition(R.anim.push_in_from_left_right,R.anim.push_out_from_right_left);
                finish();
            }else if (PreferenceUtil.getInstance(getApplicationContext()).getBooleanParam(PreferenceUtil.IS_FIRST_TIME) &&
                    PreferenceUtil.getInstance(getApplicationContext()).getBooleanParam(PreferenceUtil.IS_LOGIN_REMEMBER_ME)) {
